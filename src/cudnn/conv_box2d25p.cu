...
#define CHECK_CUDNN(expression)                             \
  ...
  int main(int argc, char *argv[]) {
    ...
    filter_data_h = (double*)malloc(1 * 1 * 5 * 5 * sizeof(double));
    ...
    double *filter_data;
    cudaMalloc(&filter_data, 1 * 1 * 5 * 5 * sizeof(double));
    cudaMemcpy(filter_data, filter_data_h, 1 * 1 * 5 * 5 * sizeof(double), cudaMemcpyHostToDevice);
