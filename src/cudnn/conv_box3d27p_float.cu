...
#define CHECK_CUDNN(expression)                             \
  ...
  int main(int argc, char *argv[]) {
    ...
    std::cout << "Cudnn, " << "3d1r_float" << ", " << L << ", " << H << ", " << W << ", " << T << ", ";
    ...
    // int T = 512;
    float *input_data_h;
    input_data_h = (float*)malloc(1 * 1 * H * W * L * sizeof(float));
    ...
    float *data[2];
    float *input_data;
    CUDA_CHECK(cudaMalloc(&input_data, 1 * 1 * H * W * L * sizeof(float)));
    CUDA_CHECK(cudaMemcpy(input_data, input_data_h, 1 * 1 * H * W * L * sizeof(float), cudaMemcpyHostToDevice));
    data[0] = input_data;
    ...
    float *filter_data_h;
    int kernel_size = 3;
    filter_data_h = (float*)malloc(1 * 1 * kernel_size * kernel_size * kernel_size * sizeof(float));

    for (int i = 0; i < kernel_size * kernel_size * kernel_size; i++) {
      filter_data_h[i] = 1.0f / 27.0f;
    }

    float *filter_data;
    CUDA_CHECK(cudaMalloc(&filter_data, 1 * 1 * kernel_size * kernel_size * kernel_size * sizeof(float)));
    CUDA_CHECK(cudaMemcpy(filter_data, filter_data_h, 1 * 1 * kernel_size * kernel_size * kernel_size * sizeof(float), cudaMemcpyHostToDevice));
    ...
    float *output_data;
    CUDA_CHECK(cudaMalloc(&output_data, outputDims[0]*outputDims[1]*outputDims[2]*outputDims[3]*outputDims[4] * sizeof(float)));
    data[1] = output_data;

    float *output_data_h;
    output_data_h = (float*)malloc(outputDims[0]*outputDims[1]*outputDims[2]*outputDims[3]*outputDims[4] * sizeof(float));
    // 执行卷积前向传播
    ...
    std::cout <<  std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count() << ", " << ((double)H * W * L) / secs / 1e9 * T << std::endl;

    CUDA_CHECK(cudaMemcpy(output_data_h, output_data, outputDims[0]*outputDims[1]*outputDims[2]*outputDims[3]*outputDims[4] * sizeof(float), cudaMemcpyDeviceToHost));
